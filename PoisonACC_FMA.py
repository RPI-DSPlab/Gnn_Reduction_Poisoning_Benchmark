...
from attack.InfMax import infmax_attack
...
def main(result, adj, features, labels):
    modified_features = infmax_attack(surrogate, adj, features, labels, idx_train, perturbations)
    ...
    for test_gnn in ['gcn', 'gat', 'sage', 'gnnguard', 'rgcn', 'median', 'strg']:
        ...
        poison_acc = eval_gnn(clean_gnn, adj, modified_features, labels, idx_test, args, test_gnn=test_gnn, device=device)
        print(f'poison_gnn: {test_gnn}, poison_acc: {poison_acc}')
                ...
                if args.reduction == 'coarsening':
                    for coarsening_method in ['variation_neighborhoods', 'variation_edges', 'variation_cliques', 'heavy_edge', 'algebraic_JC', 'kron']:
                        if test_gnn == 'rgcn':
                            ...
                            reduce_acc = eval_gnn(reduce_gnn, adj, modified_features, labels, idx_test, args, test_gnn=test_gnn, device=device)
...
if __name__ == '__main__':
    ...
    for seed in tqdm(range(1, 6)):
        ...
        result = main(result, adj, features, labels)
