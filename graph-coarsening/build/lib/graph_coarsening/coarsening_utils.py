...
def coarsen(
    ...
    for level in range(1, max_levels + 1):
        ...
        if "variation" in method:
            ...
            elif method == "variation_edges_degree":
                coarsening_list = contract_variation_edges_degree(
                    G, K=K, A=A, r=r_cur, algorithm=algorithm
                )
            else:
                if 'degree' in method:
                    coarsening_list = contract_variation_linear_degree(
...
def contract_variation_edges_degree(G, A=None, K=10, r=0.5, algorithm="greedy"):
    """
    Sequential contraction with local variation and edge-based families.
    This is a specialized implementation for the edge-based family, that works
    slightly faster than the contract_variation() function, which works for
    ...
    def adjust_weights_by_degree(deg, weights, edge):
        # 根据节点度数调整权重
        degree_factor = deg / np.max(deg)  # 归一化度数
        edge_degrees = degree_factor[edge[:2].astype(int)]  # Convert to integer
        adjustment = np.mean(edge_degrees)+1 
        # adjusted_weight = weights * (1 + adjustment)
        # adjusted_weights = []
        # print('weights ', weights.shape)
        # print('edges ', edges.shape)
        # print('zip ', zip(weights, edges))
        # for weight, edge in zip(weights, edges):
        #     edge_int = edge[:2].astype(int)  # Convert to integer
        #     print('edge_int ',edge_int)
        #     edge_degrees = degree_factor[edge_int]
        #     adjustment = np.mean(edge_degrees)  # 使用边连接的节点的平均度数进行调整
        #     adjusted_weight = weight * (1 + adjustment)  # 举例：度数较高的边权重减少
    ...
    edges = np.array(G.get_edge_list())
    weights = np.array([subgraph_cost(G, A, edges[:, e]) for e in range(M)])
    # 在使用weights之前调用adjust_weights_by_degree来调整weights
    adjustment = np.array([adjust_weights_by_degree(deg, weights, edges[:, e]) for e in range(M)])
    weights_adjusted = weights * adjustment
    # print('weights_adjusted:',weights_adjusted.shape)
    # adjust_weights_by_degree(G, weights, deg)
    # weights = np.zeros(M)
    # for e in range(M):
    #    weights[e] = subgraph_cost_old(G, A, edges[:,e])

    if algorithm == "optimal":
        coarsening_list = matching_optimal(G, weights=weights_adjusted, r=r)
    elif algorithm == "greedy":
        coarsening_list = matching_greedy(G, weights=-weights_adjusted, r=r)
...
def contract_variation_linear_degree(G, A=None, K=10, r=0.5, mode="neighborhood"):
    """
    Sequential contraction with local variation and general families.
    This is an implemmentation that improves running speed,
    at the expense of being more greedy (and thus having slightly larger error).
    ...
    def subgraph_cost(nodes):
        ...
        L = np.diag(2 * deg[nodes] - W.dot(ones)) - W
        B = (np.eye(nc) - np.outer(ones, ones) / nc) @ A[nodes, :]
        cost = np.linalg.norm(B.T @ L @ B) / (nc - 1)

        # 调整成本以考虑节点度数，对低度数节点给予优先
        degree_penalty = np.mean(deg[nodes])  # 计算节点集合的平均度数
        adjusted_cost = cost * degree_penalty  # 使用平均度数调整成本，度数越低，成本越低
...
def generate_test_vectors(
    ...
    if method == "JC" or method == "Jacobi":
        ...
        f = filters.Filter(G, lambda x: ((x <= lambda_cut) * 1).astype(float32))
