...
file_name = f'{path}/Nettack_{args.technique}_{args.gnn}.csv'
...
def coarsen_poison_test(degrees, reduce_rate, target_node):
    ...
    modified_adj, modified_features = poison(n_perturbations, target_node)
...
def jaccard_poison_test(degrees, target_node):
    ...
    model = GCNJaccard(nfeat=modified_features.shape[1],
        nhid=args.nhid,
        nclass=clean_labels.max().item() + 1,
        device=device
    )
    model = model.to(device)
    model.fit(modified_features, modified_adj, clean_labels, clean_idx_train, clean_idx_val, threshold=0.01, verbose=False)
    output = model.predict(modified_features, modified_adj)
...
def svd_poison_test(degrees, target_node):
    ...
    model = GCNSVD(nfeat=modified_features.shape[1],
        nhid=args.nhid,
        nclass=clean_labels.max().item() + 1,
        device=device
    )
    model = model.to(device)
    model.fit(modified_features, modified_adj, clean_labels, clean_idx_train, clean_idx_val, k=15, verbose=False)
    output = model.predict(modified_features, modified_adj)
    acc = (output.argmax(1)[target_node] == clean_labels[target_node])
...
def rgcn_poison_test(degrees, target_node):
    ...
    model = RGCN(
        ...
        nfeat=modified_features.shape[1],
        nhid=32,
        nclass=clean_labels.max().item() + 1,
        device=device
    )
    model = model.to(device)
    model.fit(modified_features, modified_adj, clean_labels, clean_idx_train, clean_idx_val, train_iters=200, verbose=False)
...
def median_gcn_poison_test(degrees, target_node):
    ...
    model = MedianGCN(nfeat=modified_features.shape[1],
        nhid=args.nhid,
        nclass=clean_labels.max().item() + 1,
        dropout=0.5, device=device)
    model = model.to(device)
    ...
    pyg_data.update_x(modified_features)
