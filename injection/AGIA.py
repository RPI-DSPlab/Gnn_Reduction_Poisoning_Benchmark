...
from .gia import random_injection, tdgia_injection
from .attack_util import gcn_norm, gia_update_features, init_feat, node_sim_estimate, update_features
import random
...
from .metric import *
from .injection_utils import EarlyStop
...
class AGIA(object):
    ...
    def attack(self, model, adj, features, target_idx, labels=None):
        ...
        adj = adj.to_sparse()
        adj = SparseTensor(row=adj.indices()[0], col=adj.indices()[1], value=adj.values())
        # print('adj:', type(adj))
        features_h = node_sim_estimate(features,adj,self.n_inject_max)
        adj_attack = adj
        # print('adj_attack:', type(adj_attack))
    ...
    def opt_adj_attack(self, model, adj, features, features_attack, target_idx, origin_labels, n_epoch=1e9, homophily=None):
        ...
        for i in range(self.a_epoch):
            pred = model(features_concat, adj_attack.to_dense())
